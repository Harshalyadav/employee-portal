...
import { useUpdateBranch } from "@/hooks/query/useBranch";
...
export function EditBranchForm({ branch, onSuccess }: EditBranchFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      branchAddress: {
        addressLine: branch.branchAddress?.addressLine || "",
      ...
      addressProof: branch.addressProof || "",
  ...
  const onSubmit = async (data: UpdateBranchSchema) => {
    try {
      ...
      if (submissionData.branchAddress) {
        // Preserve the nested backend address contract while avoiding empty address objects on
        // update requests when the operator leaves every address field blank.
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            {/* Address reflects the location shown in the branch list. Pincode lookup can supply
                city/state/country, but address line remains the operator-entered source of truth. */}
